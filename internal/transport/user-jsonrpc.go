...
func (http *httpUser) serveGetNameByID(ctx *fiber.Ctx) (err error) {
	return http.serveMethod(ctx, "getnamebyid", http.getNameByID)
}
func (http *httpUser) getNameByID(ctx *fiber.Ctx, requestBase baseJsonRPC) (responseBase *baseJsonRPC) {

	var err error
	var request requestUserGetNameByID
	...
	response.User, err = http.svc.GetNameByID(methodCtx, request.Id)
	if err != nil {
...
func (http *httpUser) doSingleBatch(ctx *fiber.Ctx, request baseJsonRPC) (response *baseJsonRPC) {
	...
	switch method {
	case "getnamebyid":
		return http.getNameByID(ctx, request)
	default:
