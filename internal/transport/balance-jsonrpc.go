...
func (http *httpBalance) serveGetBalance(ctx *fiber.Ctx) (err error) {
	return http.serveMethod(ctx, "getbalance", http.getBalance)
}
func (http *httpBalance) getBalance(ctx *fiber.Ctx, requestBase baseJsonRPC) (responseBase *baseJsonRPC) {

	var err error
	var request requestBalanceGetBalance
	...
	response.User, err = http.svc.GetBalance(methodCtx, request.Id)
	if err != nil {
...
func (http *httpBalance) serveMethod(ctx *fiber.Ctx, methodName string, methodHandler methodJsonRPC) (err error) {
...
func (http *httpBalance) doBatch(ctx *fiber.Ctx, requests []baseJsonRPC) (responses jsonrpcResponses) {
...
func (http *httpBalance) serveBatch(ctx *fiber.Ctx) (err error) {
...
func (http *httpBalance) doSingleBatch(ctx *fiber.Ctx, request baseJsonRPC) (response *baseJsonRPC) {

	methodNameOrigin := request.Method
	method := strings.ToLower(request.Method)
	switch method {
	case "getbalance":
		return http.getBalance(ctx, request)
	default:
